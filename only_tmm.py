# ========================================================
# Colab TMM demo (Baseline vs TMM vs LoRA) - WITH LLM TMM
#
# Run first:
...
SCALE_FACTOR = 4 if torch.cuda.is_available() else 1
WIDTH = 1024 * SCALE_FACTOR
TMM_DIM = 128 * SCALE_FACTOR
TMM_EXPAND = 2
EPOCHS = 3
BATCH_SIZE = 128 * SCALE_FACTOR
#LR_BASELINE = 1e-3
LR_TMM = 1e-3
...
MAX_SAMPLES = 1000
BATCH_SIZE_LM = 32
LR_BASELINE = 1e-3 # 2e-5
#EPOCHS = 5

# LLM Configuration
# PRETRAINED_NAME = "distilbert-base-uncased"
# LLM_SEQ_LEN = 32 * SCALE_FACTOR
# BATCH_SIZE_LM = 8
# MAX_SAMPLES = 300 * SCALE_FACTOR
...
# ================== MODEL DEFINITIONS ==================
class TMMCore(nn.Module):
    def __init__(self, in_dim=TMM_DIM, expand=TMM_EXPAND, out_dim=None):
        super().__init__()
...
class TMMLinearWrapperShared(nn.Module):
    """Shared bottleneck adapter with residual connection"""
    def __init__(self, fin, fout):
        super().__init__()
        #self.original = original  # Keep original layer
        self.core = nn.Identity()
        self.w=SharedWeight(fin, fout)
        #self.up = up
        #self.down = down
        # Learnable scaling factor for adapter output
        #self.adapter_gain = nn.Parameter(torch.tensor(0.0))

    def forward(self, x):
        # with torch.no_grad():
        #     y_orig = self.original(x)
        # adapter_out = self.down(self.core(self.up(x)))
        # return y_orig + self.adapter_gain * adapter_out
        #return self.down(self.core(self.up(x)))
        return self.core(self.w(x))
...
class SharedWeight(nn.Module):
    ...
    def stats(cls):
      print("\n===Shared Weights===")
      for w,c in cls._usage.items():
        print(f"{w} reused {c} times")

class SharedSingleLayerBank:
    """
    One Linear adapter per (in_feat, out_feat) key.
    """
    _cache = {}    
    _usage = {}
    ...
    @classmethod
    def get(cls, in_feat: int, out_feat: int):
        key = (in_feat, out_feat)
        if key not in cls._cache:
            cls._cache[key] = nn.Linear(in_feat, out_feat, bias=True)
            nn.init.xavier_uniform_(cls._cache[key].weight)
            cls._usage[key]=0
        cls._usage[key] += 1
    ...
    @classmethod
    def stats(cls):
        print("\n=== SharedSingleLayerBank Usage ===")
        for key, count in cls._usage.items():
...
class SharedAdapterBank:
    """Central repository for adapter components"""
    _ups = {}
    _downs = {}
    _usage = {}

    @classmethod
    def get(cls, in_feat, out_feat):
        # Up-projection
        if in_feat not in cls._ups:
            cls._ups[in_feat] = nn.Linear(in_feat, TMM_DIM, bias=False)
            nn.init.xavier_uniform_(cls._ups[in_feat].weight)
            cls._usage[f"up_{in_feat}"] = 0
        cls._usage[f"up_{in_feat}"] += 1

        # Down-projection
        if out_feat not in cls._downs:
            cls._downs[out_feat] = nn.Linear(TMM_DIM, out_feat, bias=False)
            nn.init.xavier_uniform_(cls._downs[out_feat].weight)
            cls._usage[f"down_{out_feat}"] = 0
        cls._usage[f"down_{out_feat}"] += 1

        return cls._ups[in_feat], cls._downs[out_feat]

    @classmethod
    def stats(cls):
        print("\n=== SharedAdapterBank Usage ===")
        for key, count in cls._usage.items():
            print(f"{key}: {count} times")
        print("===============================")
...
class TextDataset(Dataset):
    def __init__(self, split, seq_len=LLM_SEQ_LEN, max_samples=MAX_SAMPLES, validate_tokenization=True):
...
def build_vision_head(input_dim, output_dim, shared_core=None):
    if shared_core:
        return nn.Sequential(
            TMMAdapter(input_dim, WIDTH, shared_core),
            TMMAdapter(WIDTH, output_dim, shared_core)
        )
    else:
        return nn.Sequential(
...
def build_vision_tmm(in_shape, n_classes):
    shared_core = TMMCore()
    channels, *rest = in_shape
    backbone = None
...
def build_vision_lora(in_shape, n_classes, rank=4):
    ...
    class VisionWrapper(nn.Module):
        ...
        def forward(self, input_ids=None, **kwargs):
            ...
            return self.model(input_ids) if input_ids is not None else self.model(kwargs['input_ids'])
    ...
    lora_config = LoraConfig(task_type='FEATURE_EXTRACTION', r=rank, lora_alpha=rank,
...
def build_llm_baseline():
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        num_labels=2,
        torch_dtype="auto",
        use_safetensors=True,         # bypass unsafe torch.load
        ignore_mismatched_sizes=True
    )
    model.resize_token_embeddings(len(tokenizer))


    # freeze everything but last 3 layers
    model.requires_grad_(False)        
    train_layers = ["classifier", "layer.3", "layer.2"]
...
def build_llm_tmm():
    ...
    # 1. Build / attach the shared TMM core
    #core = TMMCore()
    #model.add_module("tmm_core", core)

    # 2. Wrap the chosen layers with TMM adapters (unchanged)
    #SharedSingleLayerBank.reset()
    def wrap_layer(layer):
        in_feat = layer.in_features
        out_feat = layer.out_features
        #w = SharedSingleLayerBank.get(in_feat, out_feat)
        return TMMLinearWrapperShared(in_feat, out_feat)
    ...
    # 3. Freeze everything **except** the adapters + the three target layers
    model.requires_grad_(False)
    train_keys = ["sweight", "sbias"]
    for name, p in model.named_parameters():
...
def build_llm_tmm2():
    ...
    def wrap_layer(layer):
        ...
        out_feat = layer.out_features
        w = SharedSingleLayerBank.get(in_feat, out_feat)
        return TMMLinearWrapperShared(w)
    ...
    # BERT classifier stack (no pre_classifier)
    model.classifier = wrap_layer(model.classifier)
    SharedSingleLayerBank.stats()

    # 3. Freeze everything **except** the adapters + the three target layers
    model.requires_grad_(False)
    train_keys = ["classifier", "pre_classifier", "layer.2", "layer.3",
                  "tmm_core", "adapter_gain", "up_", "down_"]
    for name, p in model.named_parameters():
        if any(k in name for k in train_keys):
            p.requires_grad_(True)
    core.unfreeze()
...
def build_llm_lora():
    config = AutoConfig.from_pretrained(PRETRAINED_NAME, num_labels=2)
    ...
    # LoRA configuration
    if hasattr(model, "distilbert"):  # DistilBERT
        target_modules = [
...
def train(model, train_loader, test_loader, lr, tag):
    ...
    def evaluate(loader):
        ...
        model.eval()
        with torch.no_grad():
            for x, y in tqdm(loader, desc="accuracy evaluation"):
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in tqdm(train_loader, desc=f"{tag} Epoch {epoch+1}/{EPOCHS}"):
            x, y = x.to(DEVICE), y.to(DEVICE)
            optimizer.zero_grad()
...
def run_experiment(name):
    ...
    print_model_stats(tmm_model, "TMM")
    print_model_stats(lora_model, "LoRA")

    # Analyze models
    b_total, b_train = count_params(baseline)
    t_total, t_train = count_params(tmm_model)
    l_total, l_train = count_params(lora_model)


    print("\nTraining TMM:")
    t_train_acc, t_test_acc, t_fwd, t_bwd, t_upd = train(
        tmm_model, train_loader, test_loader, LR_TMM, f"{name}-TMM")
    ...
    l_train_acc, l_test_acc, l_fwd, l_bwd, l_upd = train(
        lora_model, train_loader, test_loader, LR_TMM, f"{name}-LoRA")

    mem_base = get_memory_stats(baseline)
    mem_tmm = get_memory_stats(tmm_model)
    ...
    plt.plot(epochs, t_train_acc,  's--', label='TMM (train)',      color='#ff7f0e', linewidth=2.5)
    plt.plot(epochs, t_test_acc,  's-',  label='TMM (test)',       color='#ffbb78', linewidth=1.8)
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy (%)") # Add % for clarity
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.5)
    # No log scale for accuracy

    # 2. Timing breakdown
    plt.subplot(2, 2, 2)
    models = ['Baseline', 'TMM', 'LoRA']
    # Use times from the last epoch
    fwd_times = [b_fwd[-1], t_fwd[-1], l_fwd[-1]]
    bwd_times = [b_bwd[-1], t_bwd[-1], l_bwd[-1]]
    upd_times = [b_upd[-1], t_upd[-1], l_upd[-1]]
    ...
    tmm_mem = [mem_tmm[k] for k in ['param_mem', 'grad_mem', 'buffer_mem',
                                    'optimizer_state_mem', 'total_estimated_mem']]
    ...
    plt.bar(r2, tmm_mem, bar_w, label='TMM', color='#E69F00', alpha=0.8)
    ...
    plt.subplot(2, 2, 4)
    models = ['Baseline', 'TMM', 'LoRA']
    trainable = [b_train/1e6, t_train/1e6, l_train/1e6]
    total = [b_total/1e6, t_total/1e6, l_total/1e6]
    bar_w = 0.25
    x = np.arange(len(models))
    ...
    # Cleanup
    del baseline, tmm_model, lora_model
    gc.collect()
    if torch.cuda.is_available():
...
if __name__ == "__main__":
    #datasets = ['MNIST', 'FashionMNIST', 'CIFAR10', 'imdb']
    datasets = ['sst2']
